<template>
  <div class="card mb-5">
    ...
    <div class="card-body">
      ...
      <div class="card-text">
        <p>
            <span class="badge badge-dark badge-custom">{{ car.price}}$</span>
        <p>
            <span>Year: </span> {{car.year}}
        </p>
      </div>
      <router-link :to="`/car/${car.id}`" class="btn btn-dark btn-sm text-center" v-if="isLoggedIn">See More</router-link>
    </div>
  </div>
</template>
<script>
